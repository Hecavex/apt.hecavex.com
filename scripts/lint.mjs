...
const cssContract = [
  ...
  [/--space-page-top:\s*clamp\(2rem,\s*3vw,\s*3rem\);/, "missing shared page-top token"],
  [/--space-page-bottom:\s*clamp\(4rem,\s*8vw,\s*8rem\);/, "missing shared page-bottom token"],
  [/--space-major-section:\s*clamp\(3rem,\s*5vw,\s*4\.5rem\);/, "missing shared major-section token"],
  [/--frame-product-hero:\s*20rem;/, "missing open product-hero minimum height"],
  [/--display:\s*"Space Grotesk"/, "missing self-hosted display typeface"],
  [/body\s*\{[^}]*font:\s*400 1rem\/1\.65 var\(--sans\);/s, "body must retain 16px readable type"],
  ...
  [/\.brand-hero h1\s*\{[^}]*font-size:\s*var\(--type-page-title\);[^}]*line-height:\s*1\.08;/s, "product title must use the shared page-title scale"],
  [/\.page-head h1\s*\{[^}]*font-size:\s*var\(--type-page-title\);[^}]*line-height:\s*1\.08;/s, "internal page titles must use the shared title scale"],
  [/\.profile-head\s*\{[^}]*max-width:\s*64rem;/s, "profile heading measure must remain 64rem"],
  [/\.profile-head h1\s*\{[^}]*font-size:\s*var\(--type-display-title\);[^}]*line-height:\s*1\.08;/s, "profile titles must use the shared display-title scale"],
  [/\.about-summary h1\s*\{[^}]*font-size:\s*var\(--type-page-title\);[^}]*line-height:\s*1\.08;/s, "about title must use the shared page-title scale"],
  [/\.methodology-heading h1\s*\{[^}]*font-size:\s*var\(--type-page-title\);[^}]*line-height:\s*1\.08;/s, "methodology title must use the shared page-title scale"],
