...
HEIGHT = 900
ROUTES = ("/", "/about/", "/about/methodology/", "/actors/", "/actors/apt28/", "/campaigns/apt28-dns-hijacking/", "/sources/aivd-mivd-laundry-bear-2025/", "/updates/", "/search/", "/licence/", "/security/")
PROJECT_LINKS = (
    ...
    with sync_playwright() as playwright:
        ...
        for width in WIDTHS:
            ...
            for route in ROUTES:
                ...
                if route in ("/actors/apt28/", "/campaigns/apt28-dns-hijacking/"):
                    selector = ".profile-body > p" if route == "/actors/apt28/" else ".entity-profile > p:not(.eyebrow, .lead, .meta, .callout)"
                    narrative = page.locator(selector).first
                    assert narrative.count() == 1, f"long-form profile prose is missing: {route} at {width}px"
                    narrative_style = narrative.evaluate("element => ({ align: getComputedStyle(element).textAlign, hyphens: getComputedStyle(element).hyphens })")
                    assert narrative_style == {"align": "justify", "hyphens": "auto"}, f"long-form profile prose alignment drifted: {route} at {width}px: {narrative_style}"
                    lead_align = page.locator(".lead").first.evaluate("element => getComputedStyle(element).textAlign")
                    assert lead_align != "justify", f"profile lead should not be justified: {route} at {width}px"
                    first_cell = page.locator("td").first
                    if first_cell.count():
                        assert first_cell.evaluate("element => getComputedStyle(element).textAlign") != "justify", f"table cell inherited prose justification: {route} at {width}px"
                ...
                if route == "/actors/apt28/" and width > 849:
                    ...
                    profile_box = page.locator(".profile-body").bounding_box()
                    profile_copy_box = page.locator(".profile-body > p").first.bounding_box()
                    assert profile_box and profile_copy_box, f"profile prose geometry is unavailable: {route} at {width}px"
                    assert abs((profile_copy_box["x"] + profile_copy_box["width"]) - (profile_box["x"] + profile_box["width"])) <= 2, f"profile prose does not reach the contents rail: {route} at {width}px"

                if route == "/updates/" and width > 849:
                    updates_head_box = page.locator(".updates-page .page-head").bounding_box()
                    updates_lead_box = page.locator(".updates-page .lead").bounding_box()
                    assert updates_head_box and updates_lead_box, f"updates introduction geometry is unavailable: {route} at {width}px"
                    assert abs((updates_lead_box["x"] + updates_lead_box["width"]) - (updates_head_box["x"] + updates_head_box["width"])) <= 2, f"updates introduction does not span the chronological record: {route} at {width}px"
